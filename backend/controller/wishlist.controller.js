...
const toggleWishlist = async (req, res) => {
  try {
    ...
    if (!userId) {
      return res
        .status(401)
        .json({ message: "Please login to add product to wishlist" })
    }
    if (req.user.role === "admin" || req.user.role === "seller") {
      return res
        .status(400)
        .json({
          message: "Admins and Sellers cannot use the wishlist feature.",
        })
    }
    if (!productId) {
...
const createCustomWishlist = async (req, res) => {
  try {
    ...
    if (!userId) {
      return res
        .status(401)
        .json({ message: "Please login to create wishlist" })
    }
    if (!name) {
...
const toggleProductInCustomWishlist = async (req, res) => {
  try {
    ...
    const wishlist = await CustomWishlist.findOne({
      _id: customWishlistId,
      userId,
    })
    if (!wishlist)
      return res.status(404).json({ message: "Custom wishlist not found" })
    ...
    if (productIndex !== -1) {
      ...
      return res
        .status(200)
        .json({ message: "Product removed from custom wishlist", wishlist })
    } else {
      ...
      return res
        .status(200)
        .json({ message: "Product added to custom wishlist", wishlist })
    }
  } catch (error) {
    console.error(error)
    res.status(500).json({ message: "Internal server error" })
...
const getCustomWishlists = async (req, res) => {
  try {
    ...
    if (req.user.role === "admin" || req.user.role === "seller") {
      return res
        .status(200)
        .json({ message: "Custom wishlists fetched", wishlists: [] })
    }
...
const deleteCustomWishlist = async (req, res) => {
  try {
    ...
    const deletedWishlist = await CustomWishlist.findOneAndDelete({
      _id: customWishlistId,
      userId,
    })
    if (!deletedWishlist)
    ...
    return res
      .status(200)
      .json({ message: "Custom wishlist deleted successfully" })
  } catch (error) {
    console.error(error)
    res.status(500).json({ message: "Internal server error" })
  }
...
const updateCustomWishlist = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({
        message: "Custom wishlist updated successfully",
