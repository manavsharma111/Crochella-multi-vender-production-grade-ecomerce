...
const updateProfile = async (req, res) => {
  try {
    if (req.user.role === "admin" || req.user.role === "seller")
      return res.status(403).json({ message: "Cannot update mock profile" })
...
const changePassword = async (req, res) => {
  try {
    if (req.user.role === "admin" || req.user.role === "seller")
      return res
        .status(403)
        .json({ message: "Cannot change password for mock profile" })
...
const getAllDeliveryBoys = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ success: true, count: deliveryBoys.length, deliveryBoys })
  } catch (error) {
...
const assignRole = async (req, res) => {
  try {
    ...
    if (!allowedRoles.includes(role)) {
      return res
        .status(400)
        .json({ message: "Invalid role. Allowed: buyer, delivery_boy" })
    }
    ...
    res
      .status(200)
      .json({ success: true, message: `Role updated to ${role}`, user })
  } catch (error) {
...
const trackProductView = async (req, res) => {
  try {
    ...
    if (!mongoose.Types.ObjectId.isValid(productId)) {
      return res
        .status(200)
        .json({ success: true, message: "Dummy product view ignored" })
    }
...
const getRecommendations = async (req, res) => {
  try {
    ...
    if (user && user.recentlyViewed) {
      const validIds = user.recentlyViewed.filter((id) =>
    ...
    const categories = [
      ...new Set(user.recentlyViewed.map((p) => p.category).filter(Boolean)),
    ]
    const materials = [
      ...new Set(user.recentlyViewed.map((p) => p.material).filter(Boolean)),
    ]
