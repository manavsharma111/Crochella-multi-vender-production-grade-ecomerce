    ...
    try{
        ...
        if(!cart){
            const newCart = new Cart({
                userId,
                products: [{ productId, quantity: quantity || 1, price: product.price }]   
            })
            await newCart.save()
        ...
        if(item){
            ...
            cart.products.push({ productId, quantity: quantity || 1, price: product.price })
        }
...
const getCart = async(req,res) => {
    try{
        ...
        if(!cart){
            return res.status(200).json({message: "Cart fetched successfully", cart: { products: [] }, totalPrice: 0, discountAmount: 0, finalPrice: 0})
        }
        ...
        const validProducts = cart.products.filter(item => item.productId && item.productId._id);
        
        validProducts.forEach(item => {
            try {
                // Calculate dynamic price based on the current state of the populated product
                const currentDynamicPrice = calculateDynamicPrice(item.productId) || item.price || 0;
...
const applyCoupon = async (req, res) => {
    try {
        ...
        if (coupon.couponLimit <= 0) return res.status(400).json({ message: "Coupon usage limit exceeded" });
