...
const getDashboardStats = async (req, res) => {
    try {
        ...
        const revenueMetrics = orderStats[0].revenueData[0] || { totalRevenue: 0, totalPaidOrders: 0 }
        const statusBreakdown = orderStats[0].statusCounts
        const totalOrders = orderStats[0].totalOrdersData[0] ? orderStats[0].totalOrdersData[0].count : 0

        // Low Stock Check Alert Logic
        const lowStockProducts = await Product.find({ stock: { $lte: 3 } }).select('productName stock price')
