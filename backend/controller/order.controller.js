...
const placeOrder = async (req, res) => {
    try {
        ...
        const cart = await Cart.findOne({ userId }).populate('products.productId').populate('appliedCoupon');
        ...
        try {
            const deliveryBoys = await User.find({ role: 'delivery_boy', pushSubscription: { $ne: null } })
            for (const boy of deliveryBoys) {
                await sendPushAlert(boy._id, '🛵 New Order!', `Order #${order._id.toString().slice(-6)} is ready for pickup`)
            }
        } catch (notifErr) {
...
const cancelOrderNotDeliver = async (req, res) => {
    try {
        ...
        if (order.orderStatus === 'Delivered' || order.orderStatus === 'Shipped') {
            return res.status(400).json({ message: 'Cannot cancel order. Product already shipped or delivered.' })
        }
        ...
        await order.save()
        res.status(200).json({ success: true, message: 'Order cancelled & refund initiated if online paid.', data: order })
    } catch (error) {
        console.error(error)
        res.status(500).json({ message: 'Server Error' })
    }
...
const requestOrderReturn = async (req, res) => {
    try {
        ...
        if (order.orderStatus !== 'Delivered') {
            return res.status(400).json({ message: 'Can only return products that are delivered.' })
        }
        ...
        try {
            const deliveryBoys = await User.find({ role: 'delivery_boy', pushSubscription: { $ne: null } })
            for (const boy of deliveryBoys) {
                await sendPushAlert(boy._id, '↩️ Return Pickup!', `Order #${order._id.toString().slice(-6)} needs return pickup`)
            }
        } catch (notifErr) {
            console.error('Return notification error:', notifErr)
        }

        res.status(200).json({ success: true, message: 'Return request registered. Waiting for Seller approval.', data: order })
    } catch (error) {
        console.error(error)
        res.status(500).json({ message: 'Server Error' })
    }
    ...
    try {
        ...
        if (lat && lng) {
            ...
            try {
                ...
                getIo().to(`order_${id}`).emit('locationUpdate', { lat, lng, addressString, status })
            } catch (err) {
        ...
        res.status(200).json({ success: true, message: 'Order updated by seller successfully', data: order })
    } catch (error) {
        console.error(error)
        res.status(500).json({ message: 'Server Error' })
    }
    ...
    try {
        ...
        // COD Order Refund
            ...
            if (order.returnDetails.refundMethod === 'UPI') {
                console.log(`Automated Payout Triggered to UPI: ${order.returnDetails.upiId}`)
            } else {
                console.log(`Automated IMPS Transfer Triggered to Bank A/C: ${order.returnDetails.bankDetails.accountNumber}`)
            }
            order.paymentStatus = 'refunded'
        }
