...
const signUpUser = async (req, res) => {
  try {
    ...
    if (
      ...
      return res
        .status(400)
        .json({ message: "Invalid email domain or registration restricted." })
    }
...
const loginUser = async (req, res) => {
  try {
    ...
    const token = jwt.sign(
      { id: buyer._id, role: buyer.role || "buyer" },
      process.env.JWT_SECRET,
      { expiresIn: "15m" },
    )
    const refreshToken = jwt.sign(
...
const logOut = async (req, res) => {
  try {
    ...
    if (redisClient.isOpen) {
      ...
      if (token) {
        await redisClient
          .setEx(`bl_${token}`, expiry, "true")
          .catch((err) => console.error(err))
      }
      if (refreshToken) {
        await redisClient
          .setEx(`bl_${refreshToken}`, expiry, "true")
          .catch((err) => console.error(err))
      }
    }
...
const handleSocialLogin = (req, res) => {
  try {
    ...
    res.redirect(
      `${process.env.CLIENT_URL}?token=${token}&role=${req.user.role || "buyer"}`,
    )
  } catch (error) {
