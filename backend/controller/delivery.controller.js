...
const getAvailableOrders = async (req, res) => {
  try {
    const orders = await Order.find({
      ...
      .select(
        "_id createdAt totalPrice products deliveryStatus currentLocation shippingAddress paymentMethod",
      )
    ...
    res
      ...
      .json({
        success: false,
        message: "Failed to fetch pool orders",
        error: error.message,
      })
  }
}
...
const acceptForDelivery = async (req, res) => {
  try {
    ...
    res
      ...
      .json({
        success: true,
        message: "Order successfully allocated to your profile",
        data: order,
      })
  } catch (error) {
  ...
  try {
    ...
    if (!order) {
      return res
        ...
        .json({
          success: false,
          message: "Active order assignment not found for your profile",
        })
    }
      ...
      if (deliveryStatus === "Delivered") {
        if (!otp || otp !== order.deliveryOtp) {
          return res
            ...
            .json({
              success: false,
              message: "Invalid OTP provided by customer.",
            })
        }
    ...
    res
      ...
      .json({
        success: true,
        message: "Delivery status and location logs updated",
        data: order,
      })
  } catch (error) {
...
const getAvailableReturns = async (req, res) => {
  try {
    const orders = await Order.find({
      ...
      .populate("products.productId", "name price image")
      .select(
        "_id createdAt totalPrice products deliveryStatus currentLocation shippingAddress returnDetails",
      )
...
const acceptReturnPickup = async (req, res) => {
  try {
    ...
    if (!order) {
      return res
        .status(400)
        .json({ success: false, message: "Too Late! Return already accepted." })
    }
    res
      .status(200)
      .json({ success: true, message: "Return pickup accepted", data: order })
  } catch (error) {
  ...
  try {
    ...
    if (!order) {
      return res
        ...
        .json({
          success: false,
          message: "Return pickup not found for your profile",
        })
    }
    ...
    if (!otp || otp !== order.deliveryOtp) {
      return res
        .status(400)
        .json({ success: false, message: "Invalid OTP provided by customer." })
    }
    ...
    res
      ...
      .json({
        success: true,
        message: "Return item collected successfully",
        data: order,
      })
  } catch (error) {
    ...
    res
      ...
      .json({
        success: false,
        message: "Failed to mark return collected",
        error: error.message,
      })
  }
}
...
const rateDeliveryBoy = async (req, res) => {
  try {
    ...
    const newAverage =
      (currentRating.average * currentRating.count + Number(rating)) / newCount
...
const getDeliveryStaffStats = async (req, res) => {
  try {
    const User = require("../models/User")
    const deliveryBoys = await User.find({ role: "delivery_boy" }).select(
      "-password",
    )
    ...
    const stats = await Promise.all(
      deliveryBoys.map(async (dboy) => {
        ...
        const avgDeliveryTime =
          validTimeEntries > 0 ? totalDeliveryTimeHours / validTimeEntries : 0
        ...
        if (
          dboy.deliveryRating &&
          dboy.deliveryRating.count > 0 &&
          dboy.deliveryRating.average < 3.0
        ) {
          finalSalary -= baseSalary * 0.2 // 20% penalty
          penaltyReason.push("Low Rating Penalty (20%)")
        }

        // Penalty 2: Avg Delivery Time > 24 hours
        if (avgDeliveryTime > 24) {
          finalSalary -= baseSalary * 0.1 // 10% penalty
    ...
    res
      ...
      .json({
        success: false,
        message: "Failed to calculate staff stats",
        error: error.message,
      })
  }
}
