...
const addReview = async (req,res) => {
    try{
        if (req.user.role === 'admin' || req.user.role === 'seller') {
            return res.status(403).json({message: "Admins and Sellers cannot post reviews. Please login as a customer."})
        }
...
const getProductReviews = async (req,res) => {
    try{
        ...
        const reviews = await Review.find({product:productId}).populate('buyer','name email profileImage')
        res.status(200).json({
            success:true,
...
const deleteReview = async (req,res) => {
    try{
        ...
        if (req.user.id !== reviewCheck.buyer.toString() && req.user.role !== 'admin') {
