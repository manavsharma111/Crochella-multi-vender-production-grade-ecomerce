...
const deliveryBoyOrAdminMiddleware = (req, res, next) => {
    if (req.user && (req.user.role === 'delivery_boy' || req.user.role === 'admin')) {
        ...
        return res.status(403).json({ message: "Access denied. Delivery boy or Admin only." })
    }
}

module.exports = { authMiddleware, adminMiddleware, sellerMiddleware, adminOrSellerMiddleware, deliveryBoyMiddleware, deliveryBoyOrAdminMiddleware }
