...
export const getAvailableOrdersAsync = createAsyncThunk(
  "delivery/getAvailableOrders",
  async (_, { rejectWithValue }) => {
    try {
      return await deliveryService.getAvailableOrders()
    } catch (e) {
...
export const updateDeliveryStatusAsync = createAsyncThunk(
  ...
  async ({ orderId, data }, { rejectWithValue }) => {
    try {
      return await deliveryService.updateDeliveryStatus(orderId, data)
    } catch (e) {
      return rejectWithValue(e.response?.data?.message || "Failed")
    }
  },
...
export const getAvailableReturnsAsync = createAsyncThunk(
  "delivery/getAvailableReturns",
  async (_, { rejectWithValue }) => {
    try {
      return await deliveryService.getAvailableReturns()
    } catch (e) {
      return rejectWithValue(e.response?.data?.message || "Failed")
    }
  },
...
export const acceptReturnPickupAsync = createAsyncThunk(
  "delivery/acceptReturn",
  async (orderId, { rejectWithValue }) => {
    try {
      return await deliveryService.acceptReturnPickup(orderId)
    } catch (e) {
      return rejectWithValue(e.response?.data?.message || "Failed")
    }
  },
...
export const confirmReturnCollectedAsync = createAsyncThunk(
  "delivery/confirmReturn",
  async ({ orderId, data }, { rejectWithValue }) => {
    try {
      return await deliveryService.confirmReturnCollected(orderId, data)
    } catch (e) {
      return rejectWithValue(e.response?.data?.message || "Failed")
    }
  },
...
export const getMyDeliveriesAsync = createAsyncThunk(
...
const deliverySlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder
      ...
      .addCase(getAvailableOrdersAsync.fulfilled, (state, action) => {
        state.isLoading = false
      ...
      .addCase(getAvailableReturnsAsync.fulfilled, (state, action) => {
        state.isLoading = false
        state.availableReturns = action.payload.orders || []
      })
      .addCase(getAvailableReturnsAsync.rejected, handleRejected)
      .addCase(getMyDeliveriesAsync.pending, handlePending)
      .addCase(getMyDeliveriesAsync.fulfilled, (state, action) => {
        state.isLoading = false
