...
export const createCustomWishlistAsync = createAsyncThunk(
  "wishlist/createCustomWishlist",
...
export const toggleProductInCustomWishlistAsync = createAsyncThunk(
  "wishlist/toggleProductInCustomWishlist",
  async ({ customWishlistId, productId }, { rejectWithValue }) => {
    try {
      const response = await toggleProductInCustomWishlist(
        customWishlistId,
        productId,
      )
      return response
...
export const getCustomWishlistsAsync = createAsyncThunk(
  "wishlist/getCustomWishlists",
  async (_, { rejectWithValue }) => {
    try {
...
export const deleteCustomWishlistAsync = createAsyncThunk(
  "wishlist/deleteCustomWishlist",
  async (customWishlistId, { rejectWithValue }) => {
    try {
...
export const updateCustomWishlistAsync = createAsyncThunk(
  "wishlist/updateCustomWishlist",
  async ({ customWishlistId, name, description }, { rejectWithValue }) => {
    try {
      const response = await updateCustomWishlist(
        customWishlistId,
        name,
        description,
      )
...
const wishlistSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(
        toggleProductInCustomWishlistAsync.fulfilled,
        (state, action) => {
          state.isLoading = false
          state.customWishlists = state.customWishlists.map((cw) =>
            cw._id === action.payload._id ? action.payload : cw,
          )
        },
      )
      ...
      .addCase(deleteCustomWishlistAsync.fulfilled, (state, action) => {
        state.isLoading = false
        state.customWishlists = state.customWishlists.filter(
          (cw) => cw._id !== action.payload._id,
        )
      })
