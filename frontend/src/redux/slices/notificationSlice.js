...
import { subscribeUser, broadcastAlert } from "../../services/notification.service"

// subscribe user
export const subscribeUserAsync = createAsyncThunk("notification/subscribeUser", async (subscriptionData, { rejectWithValue }) => {
    try {
...
// broadcast alert
export const broadcastAlertAsync = createAsyncThunk("notification/broadcastAlert", async (alertData, { rejectWithValue }) => {
    try {
        const response = await broadcastAlert(alertData)
        return response.data
    } catch (e) {
        return rejectWithValue(e.response?.data?.message || "Failed to broadcast alert")
    }
})
