...
import { getActiveCoupons, getAllCoupons, createCoupon, deleteCoupon } from "../../services/coupon.service"

// get active coupons
export const getActiveCouponsAsync = createAsyncThunk("coupon/getActiveCoupons", async (_, { rejectWithValue }) => {
    try {
        const response = await getActiveCoupons()
        return response.data
    } catch (e) {
        return rejectWithValue(e.response?.data?.message || "Failed to fetch active coupons")
    }
})

// get all coupons
export const getAllCouponsAsync = createAsyncThunk("coupon/getAllCoupons", async (_, { rejectWithValue }) => {
    try {
        const response = await getAllCoupons()
        return response.data
    } catch (e) {
        return rejectWithValue(e.response?.data?.message || "Failed to fetch all coupons")
    }
})

// create coupon
export const createCouponAsync = createAsyncThunk("coupon/createCoupon", async (couponData, { rejectWithValue }) => {
    try {
        const response = await createCoupon(couponData)
        return response.data
    } catch (e) {
        return rejectWithValue(e.response?.data?.message || "Failed to create coupon")
    }
})

// delete coupon
export const deleteCouponAsync = createAsyncThunk("coupon/deleteCoupon", async (couponId, { rejectWithValue }) => {
    try {
        const response = await deleteCoupon(couponId)
        return response.data
    } catch (e) {
        return rejectWithValue(e.response?.data?.message || "Failed to delete coupon")
    }
})
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(deleteCouponAsync.fulfilled, (state, action) => {
                state.loading = false
                state.allCoupons = state.allCoupons.filter(c => c._id !== action.meta.arg)
            })
