    ...
    <div className="bg-[#111] p-6 rounded-2xl border-2 border-gray-800 mt-6">
      ...
      <div className="space-y-4">
        {methods.map((method) => {
          ...
          return (
            ...
            >
              <div
                className={`w-6 h-6 rounded-full border-2 flex items-center justify-center shrink-0 ${isSelected ? "border-[#ff007f] bg-[#ff007f]" : "border-gray-600"}`}
              >
      ...
      {paymentMethod === "Razorpay" && (
        <div className="mt-4 p-4 bg-[#1a1a1a] border-l-4 border-[#ff007f] text-gray-400 text-sm">
          <strong>Note:</strong> You will be redirected to the secure Razorpay
          gateway to complete your payment after clicking "Place Order".
        </div>
      )}
    </div>
  )
