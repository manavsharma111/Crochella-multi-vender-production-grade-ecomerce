...
import { getDeliveryAddressAsync, addDeliveryAddressAsync } from '../../redux/slices/userSlice'
...
const AddressSelector = ({ selectedAddress, setSelectedAddress }) => {
    ...
    const handleAddAddress = async (e) => {
        ...
        try {
            ...
            setIsAdding(false)
            setNewAddress({ Area: '', city: '', state: '', pinCode: '', country: 'India' })
            dispatch(getDeliveryAddressAsync())
        } catch (error) {
            ...
            {loading ? (
                ...
                <div className="space-y-4">
                    {addresses?.length > 0 ? (
                        <div className="grid gap-4 md:grid-cols-2">
                            {addresses.map((addr, idx) => {
                                const isSelected = selectedAddress?._id === addr._id || selectedAddress?.Area === addr.Area
                                return (
                                    <div 
                                        key={addr._id || idx}
                                    ...
                                    >
                                        <div className="flex justify-between items-start mb-2">
                                            <div className="flex items-center gap-2">
                                                <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${isSelected ? 'border-[#ff007f] bg-[#ff007f]' : 'border-gray-600'}`}>
                        ...
                        <p className="text-gray-500 italic mb-4">No saved addresses found.</p>
                    )}
                    ...
                    {!isAdding ? (
                        ...
                        <form onSubmit={handleAddAddress} className="mt-6 bg-[#0a0a0a] p-4 rounded-xl border-2 border-gray-800 space-y-4">
                            <h3 className="text-white font-bold text-sm uppercase tracking-widest mb-4">New Address</h3>
