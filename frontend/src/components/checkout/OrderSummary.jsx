...
const OrderSummary = ({ handlePlaceOrder, isPlacingOrder, isValid }) => {
    ...
    return (
        <div className="bg-[#111] p-6 rounded-2xl border-2 border-gray-800 sticky top-24">
            <h2 className="text-white font-black text-2xl uppercase tracking-widest mb-6">Review Order</h2>
            ...
            <div className="space-y-3 mb-6 max-h-48 overflow-y-auto pr-2 custom-scrollbar">
                {products.map((item) => {
                    ...
                    return (
                        <div key={product._id} className="flex gap-3 items-center bg-[#0a0a0a] p-2 rounded-lg border border-gray-800">
                            <img src={media} alt={product.productName} className="w-12 h-12 rounded object-cover" />
                            <div className="flex-1">
                                <p className="text-white text-sm font-bold line-clamp-1">{product.productName}</p>
                                <p className="text-gray-500 text-xs">Qty: {item.quantity}</p>
                            </div>
                            <div className="text-right">
                                <p className="text-white font-bold text-sm">₹{(currentPrice * item.quantity).toLocaleString('en-IN')}</p>
                            </div>
                        </div>
                    )
                })}
            ...
            <div className="space-y-4 mb-6 pt-4 border-t-2 border-gray-800">
                ...
                <div className="flex justify-between items-end">
                    ...
                    <div className="text-right">
                        <span className="text-[#ff007f] font-black text-3xl">₹{finalPrice.toLocaleString('en-IN')}</span>
            ...
            >
                {isPlacingOrder ? 'Processing...' : 'Place Order'} <ArrowRight size={20} />
            </button>
