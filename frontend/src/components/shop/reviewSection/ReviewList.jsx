...
import { deleteReviewAsync, editReviewAsync } from '../../../redux/slices/reviewSlice'
    ...
    >
      <AnimatePresence>
        {visibleReviews.map((review) => {
          const isOwner = user && review.buyer && (user.id === review.buyer._id || user._id === review.buyer._id)
            ...
            >
              ...
              {isOwner && !isEditing && (
                <div className="absolute top-4 right-4 flex gap-2">
                  <button onClick={() => startEdit(review)} className="text-gray-500 hover:text-white transition-colors p-1">
                    <Edit2 size={16} />
                  </button>
                  <button onClick={() => handleDelete(review._id)} className="text-gray-500 hover:text-red-500 transition-colors p-1">
                    <Trash2 size={16} />
                  </button>
                </div>
              )}
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4 pr-12">
                ...
                {isEditing ? (
                  <StarRating rating={editRating} setRating={setEditRating} size={16} />
                ) : (
              ...
              {isEditing ? (
                <div className="mt-4">
                  ...
                  <div className="flex justify-end gap-2">
                    <button onClick={cancelEdit} className="flex items-center gap-1 text-xs font-bold text-gray-400 hover:text-white px-3 py-1.5 rounded bg-gray-800">
                      <X size={14} /> Cancel
                    </button>
                    <button onClick={() => saveEdit(review._id)} className="flex items-center gap-1 text-xs font-bold text-white px-3 py-1.5 rounded bg-[#ff007f] hover:bg-[#d00068]">
                      <Check size={14} /> Save
                    </button>
                  </div>
                </div>
                  ...
                  {review.media && review.media.length > 0 && (
                    <div className="flex flex-wrap gap-2 mt-2">
                      {review.media.map((mediaItem, idx) => {
                        ...
                        const type = typeof mediaItem === 'string' 
                          ? (url.includes('video') || url.includes('mp4') || (url.includes('blob') && !url.includes('image')) ? 'video' : 'image') 
                          : mediaItem.type
                        
                        return (
                          <div key={idx} className="w-16 h-16 rounded-md overflow-hidden border border-gray-700 bg-black">
                            {type === 'video' ? (
