...
const ActiveCoupons = () => {
  ...
  const couponsList = Array.isArray(activeCoupons) ? activeCoupons : (activeCoupons?.data || [])
  
  // Filter out expired coupons just in case
  const validCoupons = couponsList.filter(c => new Date(c.expiryDate) > new Date())
    ...
    <div className="bg-white/5 border border-white/10 rounded-2xl p-5 w-full">
      ...
      <div className="flex flex-col gap-3">
        {validCoupons.map((coupon) => (
          <div key={coupon._id} className="relative overflow-hidden bg-black/40 border border-white/5 rounded-xl p-4 group">
            ...
            <div className="flex justify-between items-center pl-4">
              <div>
                ...
                <div className="text-xs text-gray-400">
                  {coupon.discountType === 'percentage' ? `${coupon.discountValue}% OFF` : `₹${coupon.discountValue} OFF`} - {coupon.couponName}
                </div>
              </div>
