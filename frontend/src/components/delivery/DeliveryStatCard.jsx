      ...
      <TiltCard className="bg-linear-to-br from-[#161616] to-[#0a0a0a] border border-gray-800/60 rounded-3xl p-6 relative overflow-hidden group hover:border-gray-700 hover:shadow-[0_8px_30px_rgba(0,0,0,0.5)] transition-all duration-300 h-full w-full">
        ...
        <div className="flex justify-between items-start z-10 relative">
          <div className="flex-1 min-w-0">
            ...
            <p className="text-gray-400 text-xs font-bold uppercase tracking-widest mt-2 truncate">
              {title}
            </p>
            {subtitle && (
              <p className="text-gray-600 text-[10px] uppercase font-bold tracking-wider mt-1">
                {subtitle}
              </p>
            )}
          </div>
          <div
            className={`p-4 bg-${color}-500/10 text-${color}-500 rounded-2xl border border-${color}-500/20 shadow-[0_0_15px_rgba(0,0,0,0)] group-hover:shadow-${color}-500/20 group-hover:-translate-y-1 transition-all duration-300 ml-3 shrink-0`}
          >
