...
import { MapPin, Phone, RotateCcw, IndianRupee, PackageCheck, Loader2 } from 'lucide-react'
import { toast } from 'react-hot-toast'
import { useDispatch } from 'react-redux'
import { useNavigate } from 'react-router-dom'
import { acceptReturnPickupAsync, confirmReturnCollectedAsync, getAvailableReturnsAsync, getMyDeliveriesAsync } from '../../redux/slices/deliverySlice'

const AvailableReturnCard = ({ order, isMyReturn = false, delay = 0, onOpenOtpModal }) => {
    ...
    const handleCollected = async () => {
        ...
        setLoading(true)
        try {
            const result = await dispatch(confirmReturnCollectedAsync({ orderId: order._id }))
            if (result.meta.requestStatus === 'fulfilled') {
        ...
        >
            ...
            <div className="flex items-center justify-between px-5 pt-5 pb-3 border-b border-gray-50">
                ...
                <span className="px-3 py-1 rounded-full text-xs font-bold bg-blue-100 text-blue-700">
                    {order.deliveryStatus === 'Return_Pickup' ? 'Pickup Accepted' : 'Needs Pickup'}
                </span>
            </div>
            ...
            <div className="px-5 py-4 space-y-3">
                ...
                {order.returnDetails?.reason && (
                    <div className="bg-amber-50 rounded-xl p-3 border border-amber-100">
                        ...
                        <p className="text-xs text-gray-600 mt-0.5">{order.returnDetails.reason}</p>
                    </div>
                )}
            ...
            <div className="px-5 pb-5">
                {!isMyReturn || order.deliveryStatus === 'Available' ? (
                    ...
                    <div className="flex gap-2">
                        ...
                        >
                            {loading ? <Loader2 size={18} className="animate-spin" /> : <PackageCheck size={18} />}
