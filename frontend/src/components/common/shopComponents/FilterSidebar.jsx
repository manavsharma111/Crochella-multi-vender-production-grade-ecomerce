...
const FilterSidebar = ({
  ...
  const categories = filterOptions?.categories?.length
    ? ["All", ...filterOptions.categories.filter((c) => c !== "All")]
    : ["All", "Men", "Women", "Kids", "Dress of Indian Gods"]
  const materials = filterOptions?.materials?.length
    ? filterOptions.materials
    : []
  const weaveTypes = filterOptions?.weaveTypes?.length
    ? filterOptions.weaveTypes
    : []
  ...
  const renderSidebarContent = (isMobile) => (
    ...
    >
      ...
      <div className="space-y-6 pr-2 pb-2">
        <div>
          <p className="text-[10px] font-serif text-gray-500 uppercase tracking-widest mb-3">
            Categories
          </p>
          <motion.div
            className="space-y-2"
          ...
          >
            {categories.map((category) => (
              ...
              >
                <span
                  className={`text-sm font-serif tracking-widest transition-all duration-300 ${activeCategory === category ? "text-[#FFFDD0] translate-x-2" : "text-gray-400 group-hover:text-white group-hover:translate-x-1"}`}
                >
                  {category}
                </span>
        ...
        <div>
          ...
          <div className="flex justify-between text-xs text-gray-400 font-bold">
            <span>₹0</span>
            <span className="text-[#FFFDD0] font-serif">
