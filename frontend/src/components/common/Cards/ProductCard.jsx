...
import { toggleWishlistAsync, getWishlistAsync, toggleProductInCustomWishlistAsync, getCustomWishlistsAsync } from '../../../redux/slices/wishlistSlice'
...
const ProductCard = ({ product, isListView = false }) => {
  ...
  const defaultWishlistItems = Array.isArray(wishlist) ? wishlist : (wishlist?.wishlist || [])
  const isInDefaultWishlist = defaultWishlistItems.some(item => 
    (item.productId?._id === product?._id) || (item.productId === product?._id)
  )

  // Parse custom wishlists
  const customWishlistItems = Array.isArray(customWishlists) ? customWishlists : (customWishlists?.wishlists || [])
  
  const imageUrl = product?.media?.[0]?.url || product?.images?.[0] || `https://images.unsplash.com/photo-1605518216938-7c31b7b14ad0?ixlib=rb-4.0.3&auto=format&fit=crop&w=800&q=80`
  ...
  const productName = product?.productName || product?.name || 'Handloom Silk Saree'
  ...
  const handleWishlist = (e) => {
    ...
    if(product?._id) {
      if (product._id.length !== 24) {
        toast.error("This is a demo product. Add real products from admin to use wishlist!")
        return
      }
  ...
  const handleCustomWishlistToggle = (e, customWishlistId) => {
    ...
    dispatch(toggleProductInCustomWishlistAsync({ customWishlistId, productId: product._id })).then((res) => {
      if(res.meta.requestStatus === 'fulfilled') {
  ...
  const handleAddToCart = (e) => {
    ...
    if(product?._id) {
      ...
      dispatch(addToCartAsync({ productId: product._id, quantity: 1 })).then((res) => {
        if(res.meta.requestStatus === 'fulfilled') {
  ...
  return (
    <div className={`bg-[#050505] border border-white/5 rounded-none transition-all hover:border-white/20 group flex ${isListView ? 'flex-row' : 'flex-col'} h-full relative z-0 hover:z-10`}>
      {/* Image Area */}
      <div className={`relative ${isListView ? 'w-32 md:w-48 sm:w-64 border-r border-white/5' : 'aspect-4/3 border-b border-white/5'} bg-[#0a0a0a] p-2 md:p-4 flex items-center justify-center overflow-hidden shrink-0`}>
        ...
        <div className="absolute top-2 right-2 md:top-3 md:right-3 flex flex-col gap-2 z-20">
          ...
          <div className="relative">
            <button 
              onClick={(e) => { e.preventDefault(); e.stopPropagation(); setShowCollectionMenu(!showCollectionMenu); }}
              className="p-1.5 md:p-2 bg-black/40 backdrop-blur-md border border-white/10 text-white rounded-full hover:bg-white hover:text-black transition-all active:translate-y-1"
            >
            ...
            {showCollectionMenu && (
              ...
              >
                <div className="px-3 py-2 border-b border-white/10">
                  <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Save to Collection</span>
                </div>
                <div className="max-h-40 overflow-y-auto custom-scrollbar">
                  {isAuthenticated ? (
                    customWishlistItems.length > 0 ? (
                      customWishlistItems.map(cw => {
                        const isProductInCw = cw.products?.some(p => (p._id === product?._id) || (p === product?._id))
                        return (
                          <button
                            key={cw._id}
                            onClick={(e) => handleCustomWishlistToggle(e, cw._id)}
                          ...
                          >
                            ...
                            {isProductInCw && <span className="w-2 h-2 rounded-full bg-green-500 shrink-0 ml-2"></span>}
                          </button>
                        )
                      })
                    ) : (
                      <div className="px-3 py-4 text-xs text-center text-gray-500">No collections yet</div>
                    )
                  ) : (
                    <div className="px-3 py-4 text-xs text-center text-gray-500">Please login first</div>
                  )}
                </div>
              </div>
            )}
      ...
      <div className="p-3 md:p-5 flex flex-col flex-1">
        ...
        {isListView && (
          <p className="hidden sm:block text-gray-400 text-xs md:text-sm line-clamp-2 mb-4">
            {product?.description || 'Authentic traditional hand-woven pure silk saree with intricate zari work.'}
          </p>
        )}
        ...
        <div className="flex justify-between items-center text-[8px] md:text-[10px] text-gray-500 font-bold uppercase tracking-widest mb-2 md:mb-3">
          <span className="truncate mr-2">SKU: <span className="text-gray-300">{sku}</span></span>
          <span className="shrink-0">Sold: <span className="text-gray-300">{soldOut}</span></span>
        </div>
        ...
        <div className="mb-3 md:mb-5">
          <div className="flex justify-between items-center text-[8px] md:text-[10px] text-gray-500 font-bold uppercase tracking-widest mb-1 md:mb-1.5">
            <span>Stock:</span>
            <span><span className="text-gray-300">{inStock}</span> / {totalStock}</span>
          </div>
        ...
        <div className="flex flex-col xl:flex-row items-start xl:items-center justify-between mt-auto pt-2 md:pt-4 border-t border-white/5 gap-2 xl:gap-0">
          <div className="flex items-center gap-1 md:gap-2 flex-wrap">
            <span className="text-sm md:text-xl font-serif text-[#FFFDD0]">₹{product?.price || '4,999'}</span>
            {product?.originalPrice && (
              <span className="text-[10px] md:text-sm text-gray-500 line-through">₹{product.originalPrice}</span>
            )}
          </div>
          <div className="flex gap-2 w-full xl:w-auto">
            ...
            <button 
              ...
              {isListView && <span className="hidden sm:inline">Add to Cart</span>}
              {!isListView && <span className="inline xl:hidden uppercase text-[8px] tracking-widest">ADD</span>}
            </button>
          </div>
        </div>
      </div>
