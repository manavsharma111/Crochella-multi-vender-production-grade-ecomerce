...
const FloatingChatbot = () => {
  ...
  const [chatHistory, setChatHistory] = useState([
    { sender: 'ai', text: 'Hi there! I am Baren, your AI assistant. How can I help you today?' }
  ])
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        setChatHistory(prev => [...prev, { sender: 'ai', text: response.data.message }])
      }
    } catch (error) {
          ...
          >
            ...
            <div className="bg-linear-to-r from-[#ff007f] to-[#aa0055] p-4 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-white font-bold text-lg leading-tight">Baren AI</h3>
                  <p className="text-white/80 text-xs">Customer Support Assistant</p>
                </div>
              </div>
              <button 
            ...
            {/* Chat Area */}
            <div data-lenis-prevent="true" className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar">
                ...
                >
                  <div className={`flex gap-2 max-w-[85%] ${msg.sender === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                    <div className={`w-8 h-8 rounded-full shrink-0 flex items-center justify-center mt-1 ${
                      msg.sender === 'user' ? 'bg-gray-700' : 'bg-[#ff007f]/20'
                    }`}>
                      {msg.sender === 'user' ? (
                ...
                >
                  <div className="flex gap-2 max-w-[85%] flex-row">
                    ...
                    <div className="p-3 rounded-2xl bg-white/5 rounded-tl-none border border-[#ff007f]/30 flex items-center gap-2">
                      ...
                      <span className="text-sm text-gray-400 italic">Baren is thinking...</span>
                    </div>
                  </div>
                </motion.div>
              )}
