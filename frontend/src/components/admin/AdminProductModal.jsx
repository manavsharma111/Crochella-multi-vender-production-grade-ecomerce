...
const AdminProductModal = ({ isOpen, onClose, product = null }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (product) {
        await dispatch(
          updateProductAsync({ id: product._id, productData: data }),
        ).unwrap()
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
      <div className="bg-[#1a1a1a] border border-[#ff007f]/30 rounded-3xl w-full max-w-5xl max-h-[95vh] overflow-y-auto [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-[#ff007f]/50 [&::-webkit-scrollbar-thumb]:rounded-full shadow-[0_0_50px_rgba(255,0,127,0.15)] flex flex-col">
        <div className="sticky top-0 bg-[#1a1a1a] z-10 p-5 md:p-6 border-b border-gray-800 flex justify-between items-center">
          ...
          <button
            onClick={onClose}
            className="p-2 text-gray-400 hover:text-[#ff007f] hover:bg-[#ff007f]/10 rounded-xl transition-all"
          >
            <X size={24} />
          </button>
        </div>
        ...
        >
              ...
              <div className="grid grid-cols-2 gap-4 md:gap-5">
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    className="w-full bg-[#111] border border-gray-800 rounded-xl px-4 py-2.5 text-white focus:border-[#ff007f] focus:outline-none text-sm"
            ...
            <div className="lg:col-span-5 space-y-3">
              <label className="text-xs font-bold uppercase tracking-widest text-gray-400 flex justify-between items-center">
                ...
                <span className="text-[#ff007f]">
                  {mediaFiles.filter((f) => f).length}/6 Uploaded
                </span>
              </label>
              ...
              <div className="grid grid-cols-3 gap-3">
                {mediaFiles.map((file, index) => (
                  <div
                    key={index}
                    className={`relative aspect-square border-2 border-dashed ${file ? "border-[#ff007f]/50" : "border-gray-800"} rounded-xl overflow-hidden hover:border-[#ff007f]/50 transition-colors bg-[#111] flex items-center justify-center group`}
                  >
                    <input
                      type="file"
                      accept="image/*,video/*"
          ...
          <div className="pt-5 mt-auto border-t border-gray-800 flex justify-end gap-4">
            <button
              type="button"
              onClick={onClose}
              className="px-6 py-2.5 rounded-xl font-bold text-gray-400 hover:text-white hover:bg-white/5 transition-colors text-sm"
            >
              Cancel
            </button>
            <button
              type="submit"
              disabled={isLoading}
              className="bg-[#ff007f] text-white px-8 py-2.5 rounded-xl font-black uppercase tracking-widest text-sm hover:bg-[#d6006a] hover:shadow-[0_0_20px_rgba(255,0,127,0.4)] transition-all disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {isLoading
                ? "Saving..."
                : product
