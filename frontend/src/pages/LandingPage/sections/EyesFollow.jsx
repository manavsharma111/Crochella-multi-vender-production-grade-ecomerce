  ...
  useEffect(() => {
    const handleMouseMove = (e) => {
      ...
      const moveEye = (eye, pupil, highlight) => {
        ...
        const maxDistance = rect.width / 2 - pupil.getBoundingClientRect().width / 2 - 10;
        ...
        if (highlight) {
            const maxDistance2 = pupil.getBoundingClientRect().width / 2 - highlight.getBoundingClientRect().width / 2 - 2;
      ...
      moveEye(leftEyeRef.current, leftPupilRef.current, leftHighlightRef.current);
      moveEye(rightEyeRef.current, rightPupilRef.current, rightHighlightRef.current);
    };
    ...
    <section className="w-full py-32 bg-black flex flex-col items-center justify-center border-t border-white/5 relative overflow-hidden">
      ...
      <div className="flex gap-12 md:gap-24 relative z-10">
        ...
        >
          ...
          >
             {/* Eye highlight */}
             <div ref={leftHighlightRef} className="w-4 h-4 md:w-6 md:h-6 bg-white rounded-full opacity-80 blur-[1px]"></div>
          </div>
        </div>
        ...
        >
          ...
          >
              {/* Eye highlight */}
              <div ref={rightHighlightRef} className="w-4 h-4 md:w-6 md:h-6 bg-white rounded-full opacity-80 blur-[1px]"></div>
          </div>
        </div>
      </div>
    </section>
