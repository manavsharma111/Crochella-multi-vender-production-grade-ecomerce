...
import { getMyDeliveriesAsync, updateDeliveryStatusAsync } from '../../redux/slices/deliverySlice';
...
const UserDelivery = () => {
    ...
    useEffect(() => {
        if (myDeliveries?.length > 0) {
            ...
            const active = myDeliveries.find(o => 
    ...
    const handleMarkDelivered = () => {
        ...
        dispatch(updateDeliveryStatusAsync({
            ...
            if (!res.error) {
                toast.success(isReturn ? 'Return Item Collected!' : 'Order Marked as Delivered!');
    ...
    if (isLoading && !activeOrder) {
        return <div className="min-h-screen pt-24 pb-20 flex justify-center items-center text-white bg-[#050505]">Loading...</div>;
    }

    if (!activeOrder) {
        return (
            <div className="min-h-screen pt-24 pb-20 flex justify-center items-center bg-[#050505] text-white">
                <h2 className="text-2xl font-bold uppercase tracking-widest text-gray-500">No Active Deliveries</h2>
            </div>
        );
    }
            ...
            <div className="max-w-5xl mx-auto space-y-6">
                ...
                <div className="bg-[#111] border-2 border-gray-800 rounded-3xl p-6 md:p-8 shadow-[0_10px_30px_rgba(0,0,0,0.5)]">
                    <div className="flex justify-between items-center mb-6">
                        <h2 className="text-xl font-bold flex items-center gap-2">📍 {isReturn ? 'Active Pickup Route' : 'Active Delivery Route'}</h2>
                        <p className="text-sm text-gray-400 hidden sm:block">Order ID: #{activeOrder._id}</p>
                    </div>
                    ...
                    <div className="mt-6 flex flex-col sm:flex-row justify-between items-center gap-4">
                        ...
                        <div className="flex gap-4 w-full sm:w-auto">
                            <a href={`tel:${activeOrder.userId?.phone}`} className="flex-1 sm:flex-none bg-white/10 hover:bg-white/20 px-6 py-3 rounded-xl font-bold uppercase transition-colors text-center inline-block">
                                Call
                            </a>
                            <button 
