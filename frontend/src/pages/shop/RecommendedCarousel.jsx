...
const RecommendedCarousel = ({ currentProductId }) => {
    ...
    useEffect(() => {
        const fetchRecommendations = async () => {
            try {
                const res = await axios.get(`/api/ai/recommendations/${currentProductId}`);
                if (res.data.success) {
        ...
        return (
            <div className="mt-20 border-t border-gray-900 pt-12 text-center">
                ...
                <div className="py-10 border-2 border-dashed border-gray-800 rounded-2xl flex flex-col items-center justify-center">
                    ...
                    <p className="text-gray-500 text-sm font-bold uppercase tracking-widest animate-pulse">Finding perfect matches for you...</p>
                </div>
            </div>
        );
    }
                    ...
                    >
                        <Link to={`/product/${item._id}`}>
                            <div className="w-full aspect-square bg-zinc-950 rounded-xl overflow-hidden mb-4 relative">
                                <img 
                                    src={item.media?.[0]?.url || (typeof item.media?.[0] === 'string' ? item.media[0] : item.image) || 'https://via.placeholder.com/150'} 
