...
const ProfileSidebar = ({ activeTab, setActiveTab }) => {
  ...
  const handleProfileImageChange = (e) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        dispatch(updateProfileAsync({ profileImage: reader.result })).then(
          (action) => {
    ...
    <div className="w-full md:w-64 shrink-0">
      <div className="bg-[#111] border-2 border-gray-800 rounded-3xl p-5 md:p-6 shadow-[8px_8px_0px_#000] md:sticky md:top-32 relative overflow-hidden">
        ...
        <div className="flex flex-col items-center mb-6 md:mb-8 relative z-10 pt-2">
          ...
          <h2 className="text-white font-black text-lg md:text-xl tracking-widest text-center mt-1">
            {profile?.name || "Loading..."}
          </h2>
          <p className="text-gray-400 text-[10px] md:text-xs mt-1 text-center bg-[#1a1a1a] px-3 py-1 rounded-full border border-gray-800">
            {profile?.email}
          </p>
        </div>
          ...
          {tabs.map((tab) => {
            ...
            return (
              ...
              >
                <Icon
                  size={18}
                  className="md:w-[18px] md:h-[18px] mb-0.5 md:mb-0 shrink-0"
                />
