...
import { CheckCircle2, Package, Truck, MapPin, ArrowLeft, Download, Banknote } from 'lucide-react';
...
const OrderSuccess = () => {
    ...
    const mapCenter = deliveryLocation?.coordinates && deliveryLocation.coordinates.length === 2
            ...
            <div className="max-w-5xl mx-auto space-y-8 relative z-10">
                    ...
                    <div className="flex flex-col md:flex-row items-center justify-between gap-4 mb-8 border-b border-white/10 pb-6">
                        ...
                        <div className="flex items-center gap-2 md:gap-6 text-sm font-bold uppercase tracking-widest text-gray-500 overflow-x-auto w-full md:w-auto pb-2 md:pb-0">
                            <div className="flex items-center gap-2 text-white shrink-0">
                                <span className="w-6 h-6 rounded-full bg-[#00ff88] text-black flex items-center justify-center text-xs"><CheckCircle2 size={12}/></span>
                                Cart
                            </div>
                ...
                <Reveal>
                    <div className="bg-linear-to-br from-[#111] to-[#0a0a0a] border border-white/10 rounded-3xl p-8 sm:p-12 text-center shadow-2xl relative overflow-hidden group">
                        ...
                        <h1 className="text-4xl sm:text-5xl font-black text-white uppercase tracking-widest mb-4 relative z-10">Thank You!</h1>
                        <p className="text-gray-400 font-serif text-lg mb-2 relative z-10">Your premium order has been confirmed.</p>
                        <p className="text-gray-500 font-mono text-sm mb-8 relative z-10">Order ID: #{order._id}</p>
                        ...
                        <div className="lg:col-span-2 bg-[#111] border border-gray-800 rounded-3xl overflow-hidden h-[500px] shadow-[0_10px_30px_rgba(0,0,0,0.3)] relative">
                            {orderStatus === 'Delivered' ? (
                                <div className="absolute inset-0 z-10 bg-black/60 backdrop-blur-sm flex flex-col items-center justify-center">
                                    ...
                                    <h3 className="text-2xl font-black text-white uppercase tracking-widest">Package Delivered</h3>
                                </div>
                            ) : null}
                            
                            <LiveTrackingMap 
                                deliveryCoords={{ lat: mapCenter[0], lng: mapCenter[1] }}
                                userCoords={order?.shippingAddress?.coordinates ? { lat: order.shippingAddress.coordinates[1], lng: order.shippingAddress.coordinates[0] } : null}
                            />
                        </div>
                                ...
                                <div className="mb-8 mt-4 shrink-0 relative px-4">
                                    {(() => {
                                        ...
                                        if (!isReturn) {
                                            ...
                                            if (orderStatus === 'Out_for_Delivery' || orderStatus === 'Out for delivery') currentStep = 3;
                                            if (orderStatus === 'Delivered') currentStep = 4;
                                        } else {
                                            if (orderStatus === 'Return_Accepted') currentStep = 2;
                                            if (orderStatus === 'Return_Pickup' || orderStatus === 'Out for Pickup') currentStep = 3;
                                            if (orderStatus === 'Return_Collected' || orderStatus === 'Returned' || orderStatus === 'Collected') currentStep = 4;
                                            if (orderStatus === 'Refunded') currentStep = 5;
                                        }

                                        let fillPercent = order?.progressPercent > 0 ? order.progressPercent : ((currentStep - 1) / (steps.length - 1)) * 100;
                            ...
                            {deliveryBoyId && (
                                <div className="bg-[#111] border border-gray-800 rounded-3xl p-6 shadow-[0_10px_30px_rgba(0,0,0,0.3)]">
                                    <h3 className="text-white font-bold uppercase tracking-widest mb-4 flex items-center gap-2">
                                        <MapPin size={18} className="text-[#00ff88]" /> Delivery Partner
                                    </h3>
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            <p className="text-white font-bold">{deliveryBoyId?.name || 'Assigned Partner'}</p>
                                            <p className="text-gray-400 text-sm mt-0.5">{deliveryBoyId?.phone || 'Contact info pending'}</p>
                                        </div>
                                    </div>
                                </div>
                            )}
