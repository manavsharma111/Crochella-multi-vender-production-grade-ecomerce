        ...
        <div className="space-y-6 max-w-7xl mx-auto">
            ...
            {isLoading ? (
                ...
                >
                    ...
                    <p className="text-gray-700 font-semibold text-lg">No orders available</p>
                    <p className="text-gray-500 text-sm mt-1">New orders will appear here when customers place them</p>
                </motion.div>
            ) : (
                <>
                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
                        {availableOrders.slice(0, visibleCount).map((order, i) => (
                            <AvailableOrderCard key={order._id} order={order} delay={i * 0.07} />
                        ))}
                    </div>
                    <div className="flex justify-center gap-4 mt-8">
                        {visibleCount < availableOrders.length && (
                            <button 
                                onClick={() => setVisibleCount(prev => Math.min(prev + 6, availableOrders.length))}
