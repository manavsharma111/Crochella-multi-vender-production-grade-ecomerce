                    ...
                    <div className="flex flex-col md:flex-row items-center justify-between gap-4 mb-12 border-b border-white/10 pb-6">
                        ...
                        <div className="flex items-center gap-2 md:gap-6 text-sm font-bold uppercase tracking-widest text-gray-500 overflow-x-auto w-full md:w-auto pb-2 md:pb-0">
                            <div className="flex items-center gap-2 text-white shrink-0">
                                <span className="w-6 h-6 rounded-full bg-[#ff007f] text-white flex items-center justify-center text-xs">1</span>
                                Shopping Cart
                            </div>
                ...
                {isLoading && products.length === 0 ? (
                    ...
                    <Reveal>
                        <div className="bg-[#111] border-2 border-gray-800 rounded-3xl p-12 text-center flex flex-col items-center">
                            ...
                            <h2 className="text-white font-black text-2xl uppercase tracking-widest mb-4">Your Cart is Empty</h2>
                            <p className="text-gray-400 max-w-md mb-8">Looks like you haven't added anything to your cart yet. Discover our premium handloom collection and find something you love.</p>
                            <button 
                                onClick={() => navigate('/shop')}
                        ...
                        <div className="flex-1 w-full space-y-6">
                            ...
                            <div className="space-y-4 max-h-[50vh] overflow-y-auto pr-2 custom-scrollbar">
                                {products.map((item, idx) => (
                                    <CartItem key={item._id || item.productId?._id || item.productId || idx} item={item} />
                                ))}
                            </div>
                            ...
                            <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-gray-400 py-4 border-b border-white/10">
                                <span className="text-white">🔒 Buy With Confidence</span> • 100% NO-RISK MONEY BACK GUARANTEE
                                <button className="ml-auto text-red-500 hover:text-red-400 transition-colors flex items-center gap-1">
                                    <span className="border border-red-500 rounded-full w-4 h-4 flex items-center justify-center text-[10px]">✕</span> Cancel Order
                                </button>
                            </div>
                            ...
                            {flashSales && flashSales.length > 0 && (
                                <div className="pt-8">
                                    <h3 className="text-white font-bold text-lg uppercase tracking-widest mb-6">You may also be interested in these exclusive offers.</h3>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        {flashSales.slice(0, 2).map((item) => (
                                            <div key={item._id} className="border border-white/10 bg-[#0a0a0a] rounded-2xl p-4 flex gap-4 items-center group hover:border-[#ff007f]/30 transition-all relative z-10">
                                                <div className="w-20 h-20 bg-black rounded-xl overflow-hidden shrink-0">
                                                    <img src={item.media?.[0]?.url || '/placeholder.png'} alt="cross-sell" className="w-full h-full object-cover group-hover:scale-110 transition-transform" />
                                                </div>
                                                <div className="flex-1 min-w-0">
                                                    <h4 className="text-white font-bold truncate pr-2">{item.productName || item.name || 'Exclusive Product'}</h4>
                                                    <p className="text-gray-500 text-[10px] line-clamp-2 pr-2 mt-1">{item.description}</p>
                                                    <div className="flex justify-between items-center mt-3 relative z-20">
                                                        <span className="text-[#ff007f] font-bold">₹{item.price}</span>
                                                        <button 
                            ...
                            <div className="mt-8 pt-8 border-t-2 border-gray-800">
                                <h3 className="text-white font-bold text-xl uppercase tracking-widest mb-4">Available Offers</h3>
                                <ActiveCoupons />
                            </div>
                        </div>
