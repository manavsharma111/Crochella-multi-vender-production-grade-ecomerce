...
import {
  ...
  sellerGetAllOrdersAsync,
  handleOrderRefundAsync,
...
const OrderManagementModal = ({ order, onClose }) => {
  ...
  const [addressString, setAddressString] = useState(
    order.currentLocation?.addressString || "",
  )
      ...
      (t) => (
        <div>
          <p className="font-bold mb-3 text-white">
            Are you sure you want to process refund and restock this order?
          </p>
            ...
            {order.returnDetails && (
              <div className="p-4 bg-[#1a1a1a] border border-gray-800 rounded-xl space-y-3">
                ...
                <div className="space-y-2">
                  ...
                  {order.returnDetails.bankDetails &&
                    order.returnDetails.bankDetails.accountNumber && (
                      <div className="bg-[#111] p-3 rounded-lg border border-gray-800 space-y-1">
                        <p className="text-sm text-white">
                          <span className="text-gray-500 text-xs">Name:</span>{" "}
                          {order.returnDetails.bankDetails.accountHolderName}
                        </p>
                        <p className="text-sm text-white">
                          <span className="text-gray-500 text-xs">A/C:</span>{" "}
                          {order.returnDetails.bankDetails.accountNumber}
                        </p>
                        <p className="text-sm text-white">
                          <span className="text-gray-500 text-xs">IFSC:</span>{" "}
                          {order.returnDetails.bankDetails.ifscCode}
                        </p>
                      </div>
                    )}
                </div>
              ...
              {status === "Returned" && order.paymentStatus !== "refunded" && (
                ...
                >
                  {isRefunding ? (
                    ...
                    <>
                      <RefreshCcw size={18} /> Refund & Restock
                    </>
                  )}
                </button>
              )}
