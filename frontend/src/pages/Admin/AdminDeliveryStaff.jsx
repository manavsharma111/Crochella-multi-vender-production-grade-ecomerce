...
import { User, Shield, AlertTriangle, Clock, TrendingDown, Search } from 'lucide-react'
    ...
    return (
        <div>
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
                <h1 className="text-3xl font-black text-white uppercase tracking-widest">Delivery Staff</h1>
            </div>
                        ...
                        <tbody>
                            {loading ? (
                                ...
                                filteredStaff.slice(0, visibleCount).map((s) => (
                                    <tr key={s._id} className="border-b border-gray-800/50 hover:bg-gray-800/20 transition-colors">
                                        ...
                                        <td className="px-6 py-4">
                                            <span className="text-white font-black text-lg">{s.totalDeliveries}</span>
                                            <span className="text-gray-500 text-xs ml-1 uppercase font-bold">Total</span>
                                        </td>
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-2">
                                                <Clock size={16} className={Number(s.avgDeliveryTime) > 24 ? "text-red-500" : "text-green-500"} />
                                                <span className={`font-bold ${Number(s.avgDeliveryTime) > 24 ? "text-red-500" : "text-white"}`}>
                                        ...
                                        <td className="px-6 py-4">
                                            {s.deliveryRating?.average > 0 ? (
                                                <div className={`flex items-center gap-1 w-fit px-2 py-0.5 rounded border font-bold text-xs ${s.deliveryRating.average < 3.0 ? 'text-red-500 bg-red-500/10 border-red-500/20' : 'text-yellow-500 bg-yellow-500/10 border-yellow-500/20'}`}>
                                                    ⭐ {s.deliveryRating.average.toFixed(1)} / 5.0
                                                </div>
                                            ) : (
                                                <span className="text-gray-500 text-xs font-bold uppercase">No Ratings</span>
                                            )}
                                        </td>
                                        <td className="px-6 py-4">
                                            <div className="flex flex-col">
                                                <span className="text-white font-black text-lg">₹{s.finalSalary.toLocaleString('en-IN')}</span>
                                                {s.penalties && s.penalties.length > 0 && (
                                                    <div className="flex flex-col gap-1 mt-1">
                                                        {s.penalties.map((penalty, idx) => (
                                                            <span key={idx} className="flex items-center gap-1 text-[10px] text-red-500 font-bold uppercase tracking-wider">
