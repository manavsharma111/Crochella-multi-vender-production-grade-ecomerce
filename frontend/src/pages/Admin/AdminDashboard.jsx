...
const StatCard = ({ title, value, icon: Icon, color }) => {
  ...
  return (
    <TiltCard className="bg-linear-to-br from-[#161616] to-[#0a0a0a] border border-gray-800/60 rounded-3xl p-6 relative overflow-hidden group hover:border-gray-700 hover:shadow-[0_8px_30px_rgba(0,0,0,0.5)] transition-all duration-300">
      <div
        className={`absolute -right-12 -top-12 w-40 h-40 bg-${color}-500/10 rounded-full blur-3xl group-hover:bg-${color}-500/20 transition-all duration-500`}
      ></div>
      <div className="flex justify-between items-start z-10 relative">
        ...
        <div
          className={`p-4 bg-${color}-500/10 text-${color}-500 rounded-2xl border border-${color}-500/20 shadow-[0_0_15px_rgba(0,0,0,0)] group-hover:shadow-${color}-500/20 group-hover:-translate-y-1 transition-all duration-300`}
        >
      ...
      <div className="mt-8 flex items-end gap-1.5 h-10 z-10 relative opacity-40 group-hover:opacity-80 transition-opacity duration-300">
        {bars.map((height, i) => (
          <div
            key={i}
            className={`flex-1 bg-${color}-500/60 rounded-t-md hover:bg-${color}-400 transition-colors cursor-pointer`}
            style={{ height: `${height}%` }}
          ></div>
        ))}
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {Array(4)
            ...
            .map((_, i) => (
              <div
                key={i}
                className="bg-[#111] border border-gray-800 rounded-3xl p-6 h-36 animate-pulse"
              ></div>
            ))}
        </div>
          ...
          {lowStockProducts?.length > 0 ? (
            <div className="space-y-3">
              {lowStockProducts.map((product) => (
                <div
                  key={product._id}
                  className="flex justify-between items-center p-4 bg-[#111]/50 hover:bg-[#111] rounded-2xl border border-gray-800/50 transition-colors group"
                >
                  ...
                  <div className="text-right bg-red-500/10 px-4 py-2 rounded-xl border border-red-500/20">
                    ...
                    <p className="text-red-500/70 text-[10px] uppercase tracking-widest font-bold">
                      Left
                    </p>
                  </div>
                </div>
                  ...
                  <div key={status._id} className="group cursor-pointer">
                    ...
                    <div className="w-full h-3 bg-[#111] rounded-full overflow-hidden shadow-inner border border-gray-800/50">
                      <div
                        ...
                        style={{
                          width: `${(status.count / metrics.totalOrders) * 100}%`,
                        }}
                      ></div>
                    </div>
                  </div>
