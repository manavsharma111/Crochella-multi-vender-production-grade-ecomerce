describe('Return and Refund Flow', () => {
  it('Should let a user request a return with UPI details, and let admin process the refund', () => {
    ...
    cy.get('button').contains(/sign in/i).first().click()
    ...
    cy.contains(/delivered/i).parents('.bg-\\[\\#1a1a1a\\]').within(() => {
      cy.get('button').contains(/return/i).first().click()
    })
    ...
    // Submit Return
    cy.get('button').contains(/submit return/i).click()
    ...
    cy.contains(/returned/i).parents('tr').within(() => {
    ...
    cy.get('button').contains(/refund & restock/i).click()
